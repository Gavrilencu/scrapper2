...
def list_email_configs():
    ...
    try:
        rows = db.execute(
            "SELECT id, name, host, port, secure, user_name, from_address, created_at FROM email_config ORDER BY name"
        ).fetchall()
...
def create_email_config(body: EmailConfigCreate):
    ...
    try:
        cur = db.execute(
            ...
            (body.name, body.host, body.port, 1 if body.secure else 0, body.user_name, body.password, body.from_address),
        )
        db.commit()
    ...
    try:
        ...
        if body.password:
            db.execute(
                ...
                (name, host, port, 1 if secure else 0, user_name, body.password, from_address, id),
            )
        else:
            db.execute(
                "UPDATE email_config SET name=?, host=?, port=?, secure=?, user_name=?, from_address=? WHERE id=?",
                (name, host, port, 1 if secure else 0, user_name, from_address, id),
            )
        db.commit()
        return {"ok": True}
    finally:
